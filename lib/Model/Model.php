 ...
 */
	...
	public function save($data = null, $params = array(), $validate = true) {
		if(isset($params['saveMany'])){
			...
			if($validate && isset($params['deep'])){
				if(!$this->validateDeep($data, $on))return false;
			...
			foreach ($this->schema as $col=>$colInfo){
				if(isset($data[$this->alias][$col])){
					...
					}elseif($colInfo['type']=='datetime' && $data[$this->alias][$col]<>'0000-00-00 00:00:00'){
						$this->data[$this->alias][$col] = date('Y-m-d H:i:s', strtotime($data[$this->alias][$col]));
					}elseif($colInfo['type']=='date' && $data[$this->alias][$col]<>'0000-00-00 00:00:00'){
			...
			if((is_string($als) && $this->alias==$als) || is_int($als)){
				foreach ($d as $alias=>$saveD){
					if((is_int($als) && $this->alias==$alias) || is_int($alias)){
						...
						if($validate && !$this->validate($saveData, $on)){
							$error = true;
						}
						$dataArray[] = $saveData;
					}
	...
	public function validate(&$data = array(), $on = 'update', $validator = null){
		...
		foreach ($this->schema as $colName=>$colInfo){
				...
				foreach ($validator[$colName] as $v){
					if($on==$v['on'] || $v['on']===null){
						if(isset($data[$this->alias][$colName])){
							...
							if($v['allowEmpty']==false && (empty($data[$this->alias][$colName]) || !is_numeric($data[$this->alias][$colName]))){
								$error = true;
							}
						}else{
							if($v['require']==true){
						...
						if($error){
							$this->validateErrors[$this->alias][$colName][] = $v['message'];
			...
			if(isset($this->validator[$colName])){
				if(is_array($this->validator[$colName][0])){
					foreach ($this->validator[$colName][0] as $validator){
						$this->validate($data, $on, $validator);
					}
				}else{
					if(validator === null){
						$rules = array_merge($default, $this->validator[$colName]);
					}else{
						$rules = array_merge($default, $validator);
					}
					if($rates['on']===null || ($rates['on']==$on)){
						if(array_key_exists($rules['rule'], $rulesRegEx)){
							$rules['rule'] = $rulesRegEx[$rules['rule']];
						}
						if(!preg_match($rules['rule'], $data[$colName])){
							if($rules['allowEmpty']==false && $data[$colName]==''){
								$error = true;						
							}
						}
						if(!isset($data[$colName]) && $rules['required']){
								$error = true;								
						}
					}					
					if($error) $this->validateErrors[$this->name][$colName] = $rules['message'];
				}
			}*/
