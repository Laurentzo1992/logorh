...
class AffdotitemsController extends AppController{	
	var $paginate = array(
		'Affdotitem'=>array(
			'model'=>'Affdotitem','sort'=>'id', 'affdotitem'=>'ASC',
	...
	var $uses = array('User','Agdossier','Parampharmacie','Affbonpharma','Paramregimemedico','Agcontrat','Signataire','Affdotverre');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Affdotitems'));
		if($accessLevel['view']){
			$this->set('accessLevel', $accessLevel);
		}

		$postData = $this->postData();
        $dotverre_id = $this->getGetParam('id');
        /********************************************************************/
	    ...
	    $tmp = $this->Affdotverre->find('all', array('conditions'=>array('Affdotverre.id='.$dotverre_id), 'recursive'=>0));
	    $dot_id = $tmp[0]['Affdotverre']['id'];
		$agdossier_id = $tmp[0]['Affdotverre']['agdossier_id'];
		$dotation_restant = $tmp[0]['Affdotverre']['dotation_restant'];
		/******************************************************/

		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOTATION VERRE CORRECTEUR <span class="pageTitle">'.$name . SEP . $username.'</span>');

		$this->paginate['Affdotitem']['conditions'][] = array('Affdotitem.agdossier_id'=>$agdossier_id);
		$this->set('affdotitems', $this->paginate('Affdotitem'));
	...
	public function edit(){
		...
		$dotverre_id = $this->getGetParam('dotverre_id');
		$agdossier_id = $this->getGetParam('agdossier_id');
		$dotation_restant = $this->getGetParam('dotation_restant');
		
		$postData = $this->postData();

        //print_r($agcontrats);
		if(isset($postData['Affdotitem']['submit']) && isset($postData['Affdotitem'])){
            if($postData['Affdotitem']['affdotverre_id']<>'' &&
               $postData['Affdotitem']['agdossier_id']<>'' &&
               $postData['Affdotitem']['date_item']<>'' &&
               $postData['Affdotitem']['libelle']<>'' &&
               $postData['Affdotitem']['montant']<>'')
            {
			    /*===========*/
			    $montant = $postData['Affdotitem']['montant'];
	           
				if($montant <= $dotation_restant){
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Affdotitem ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){
					    /*==================================*/
					    $dotation_restant = $dotation_restant - $montant;
					     $dotverre = array('Affdotverre'=>array(
						        'id' => $dotverre_id,
						        'dotation_restant' => $dotation_restant,
								   )
					            );
						  $this->Affdotverre->save($dotverre);
					    /*==================================*/				
						$saveId = $this->Affdotitem->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affdotitems', 'view'=>'index', 'params'=>array('id:'.$dotverre_id)));
							}else{
								$this->redirect(array('controller'=>'Affdotitems', 'view'=>'index', 'params'=>array('id:'.$dotverre_id)));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Affdotitem->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affdotitems', 'view'=>'index', 'params'=>array('id:'.$dotverre_id)),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DOTATION PRODUITS PHARMACEUTIQUES':'MODIFICATION  DOTATION PRODUITS PHARMACEUTIQUES'));
		...
		$this->set('parampharmacies', $this->Parampharmacie->find('list', array('list'=>array('id','nom_pharma'), 'order'=>'id ASC')));
         $tmp = $this->Paramregimemedico->find('all', array('conditions'=>array('Paramregimemedico.id=9'), 'recursive'=>0));
		$dotation = $tmp[0]['Paramregimemedico']['valeur'];
	   $this->set('dotation', $dotation);

	    $this->set('dotverre_id', $dotverre_id);
		$this->set('agdossier_id', $agdossier_id);
		$this->set('dotation_restant', $dotation_restant);
	...
	public function modif(){
		...
		if(isset($postData['Affdotitem']['submit']) && isset($postData['Affdotitem'])){
            if($postData['Affdotitem']['annee']<>'' &&
               $postData['Affdotitem']['matricule']<>'' &&
               $postData['Affdotitem']['dotation']<>'' &&
               $postData['Affdotitem']['dotation_restant']<>'')
            {
					...
					if($accessLevel['view'] && $accessLevel['edit']){				
						...
						if($saveId){
							...
							$this->Session->setFlash('Enregistré avec succès');
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affdotitems', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Affdotitems', 'view'=>'index'));
							}
						}else {
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affdotitems', 'view'=>'index', 'params'=>array()),
				'options'=>array('class'=>'toolbarItem back')
			);
		}else{
			$toolbar['Retour'] = array(
		...
		$this->set('parampharmacies', $this->Parampharmacie->find('list', array('list'=>array('id','nom_pharma'), 'order'=>'id ASC')));
         $tmp = $this->Paramregimemedico->find('all', array('conditions'=>array('Paramregimemedico.id=7'), 'recursive'=>0));
		$dotation = $tmp[0]['Paramregimemedico']['valeur'];
	   $this->set('dotation', $dotation);
	...
	public function traitement() {
		...
		$agdossier = 2;
		 //montant_pret
        /*---------------------------------*/
		if(isset($postData['Affdotitem']['valider'])){
			$datedebut = $postData['Affdotitem']['date_debut'];
			$datefin = $postData['Affdotitem']['date_fin'];
			$agdossier = $postData['Affdotitem']['agdossier_id'];
		    $this->data = $postData;
		}
	   ...
	   /*--------------------------------*/
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DOTATION BON':'MODIFICATION  DOTATION BON'));
		$this->set('toolbar', $toolbar);
	...
	public function index1() {
        ...
        /******************************************************/
		if(isset($postData['Affdotitem']['valider'])){
			$annee = $postData['Affdotitem']['annee'];
		    $this->data = $postData;
		}
		/******************************************************/

		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOTATION PRODUITS PHARMACEUTIQUES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		 $this->paginate['Affdotitem']['conditions'][] = array('Affdotitem.annee'=>$annee);
		$this->set('affdotitems', $this->paginate('Affdotitem'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DOTATION PRODUITS PHARMACEUTIQUES'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Affdotitem->find('all', array('conditions'=>array(array($this->Affdotitem->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Affdotitems';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Affdotitem']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Affdotitem->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES DOTATIONS PRODUITS PHARMACEUTIQUES'));
		//$this->set('toolbar', $toolbar);
		$this->set('Affdotitems', $this->paginate('Affdotitem'));
