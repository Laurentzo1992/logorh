...
class ParamindemitemsController extends AppController{	
	var $paginate = array(
		'Paramindemitem'=>array(
			'model'=>'Paramindemitem','sort'=>'id', 'Paramindemitem'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramclassification','Paramindemnite','Paramtypefonction');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramindemitems'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'MONTANT/TAUX ELEMENT DE SALAIRE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramindemitems', $this->paginate('Paramindemitem'));
        ...
        $this->set('paramtypefonctions', $this->Paramtypefonction->find('list', array('list'=>array('id','libelle'), 'order'=>'id ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Paramindemitem']['submit']) && isset($postData['Paramindemitem'])){
            if($postData['Paramindemitem']['paramindemnite_id']<>''  &&
               $postData['Paramindemitem']['montant']<>'' ||
               $postData['Paramindemitem']['taux']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramindemitem ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramindemitem->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramindemitems', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramindemitems', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramindemitem->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramindemitems', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION MONTANT/TAUX ELEMENT DE SALAIRE':'MONTANT/TAUX ELEMENT DE SALAIRE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE GRILLE SALARIALE '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramindemitem->find('all', array('conditions'=>array(array($this->Paramindemitem->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramindemitems';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramindemitem']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramindemitem->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('GRILLE SALARIALE'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramindemitems', $this->paginate('Paramindemitem'));
