...
class AgrupturesController extends AppController{	
	var $paginate = array(
		'Agrupture'=>array(
			'model'=>'Agrupture','sort'=>'id', 'agrupture'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Agcontrat','Paramtyperupture');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Agruptures'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'RUPTURES DE CONTRATS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('agruptures', $this->paginate('Agrupture'));
		
		

		$this->set('motifs', $this->Paramtyperupture->find('list', array('list'=>array('id','libelle'), 'order'=>'id ASC')));
	...
	public function edit() {
		...
		$postData = $this->postData();
		 $dossierID = 'RAS';
		/******************************************************/
		if(isset($postData['Agrupture']['valider'])){
			$dossierID = $postData['Agrupture']['agdossier_id'];
			$this->data = $postData;
		}
		/******************************************************/
		if(isset($postData['Agrupture']['submit']) && isset($postData['Agrupture'])){
            if($postData['Agrupture']['agdossier_id']<>'' &&
               $postData['Agrupture']['agcontrat_id']<>'' &&
               $postData['Agrupture']['date_rupture']<>'' &&
               $postData['Agrupture']['date_rupture']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Agrupture ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){	

					    /*-----------------------------------*/
	                    $dos = $this->Agdossier->find('all', array('conditions'=>array('Agdossier.id='.$postData['Agrupture']['agdossier_id']), 'recursive'=>0));
			            $matricule = $dos[0]['Agdossier']['ag_matricule'];
	                    /*-----------------------------------*/	
	                    $postData['Agrupture']['matricule']  = $matricule;

	                    /*-----------------------------------*/
	                    $con = $this->Agcontrat->find('all', array('conditions'=>array('Agcontrat.id='.$postData['Agrupture']['agcontrat_id']), 'recursive'=>0));
			            $numero = $con[0]['Agcontrat']['num_contrat'];
	                    /*-----------------------------------*/	
	                    $postData['Agrupture']['numcontrat']  = $numero;				
						$saveId = $this->Agrupture->save($postData);
						/*=========================*/
						$agruptureid = $this->Agrupture->id;
						 $contrat = array('Agcontrat'=>array(
							          'id' => $postData['Agrupture']['agcontrat_id'],
						              'statut' => 0,
						           
									   )
						            );
							    $this->Agcontrat->save($contrat);	
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Agruptures', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Agruptures', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Agrupture->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Agruptures', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION RUPTURE DE CONTRAT':'MODIFICATION RUPTURE DE CONTRAT'));
        ...
        $this->set('agcontrats', $this->Agcontrat->find('list', array('list'=>array('id', 'num_contrat'), 'conditions'=>array("Agcontrat.agdossier_id = '{$dossierID}'"),'order'=>'num_contrat ASC')));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE RUPTURE DE CONTRAT'));
		$this->set('toolbar', $toolbar);
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Agrupture->find('all', array('conditions'=>array(array($this->Agrupture->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Agruptures';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Agrupture']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Agrupture->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES RUPTURES DE CONTRATS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Agruptures', $this->paginate('Agrupture'));
