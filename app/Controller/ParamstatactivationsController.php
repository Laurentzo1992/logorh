...
class ParamstatactivationsController extends AppController{	
	var $paginate = array(
		'Paramstatactivation'=>array(
			'model'=>'Paramstatactivation','sort'=>'libelle', 'paramstatactivation'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramstatactivations'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'STATUT ACTIVATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramstatactivations', $this->paginate('Paramstatactivation'));
	...
	public function edit() {
		...
		if(isset($postData['Paramstatactivation']['submit']) && isset($postData['Paramstatactivation'])){
            if($postData['Paramstatactivation']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramstatactivation ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramstatactivation->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramstatactivations', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramstatactivations', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramstatactivation->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramstatactivations', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION STATUT ACTIVATION':'MODIFICATION STATUT ACTIVATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE STATUT ACTIVATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramstatactivation->find('all', array('conditions'=>array(array($this->Paramstatactivation->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramstatactivations';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramstatactivation']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramstatactivation->delete($ids);
