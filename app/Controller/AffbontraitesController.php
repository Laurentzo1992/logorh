...
class AffbontraitesController extends AppController{	
	var $paginate = array(
		'Affbontraite'=>array(
			'model'=>'Affbontraite','sort'=>'id', 'affbontraite'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Affbonretenue');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Affbontraites'));
		if($accessLevel['view']){
       ...
       // retid
        ...
        $tmp = $this->Affbonretenue->find('all', array('conditions'=>array('Affbonretenue.id='.$retid), 'recursive'=>0));
        $id = $tmp[0]['Affbonretenue']['id'];
		$agdossier_id = $tmp[0]['Affbonretenue']['agdossier_id'];
		$montant_ret = $tmp[0]['Affbonretenue']['montant_ret'];
        /********************************************************************/
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TRAITES MENSUELS BON PRODUITS PHARMACEUTIQUES<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->paginate['Affbontraite']['conditions'][] = array('Affbontraite.affbonretenue_id'=>$retid);
		$this->set('affbontraites', $this->paginate('Affbontraite'));
        
        $this->set('toolbar',$toolbar);
        $this->set('retid',$retid);
        $this->set('agdossier_id',$agdossier_id);
        $this->set('montant_ret',$montant_ret);
...
$this->set('affbonretenues', $this->Affbonretenue->find('list', array('list'=>array('id','montant_ret'), 'order'=>'id ASC')));
	...
	public function edit() {
		...
		$postData = $this->postData();
		 ...
		 //montant_ret
        /******************************************************/
		if(isset($postData['Affbontraite']['valider'])){
			$nbrtraite = $postData['Affbontraite']['nbr_traite'];
			$montantpret = $postData['Affbontraite']['montant_ret'];
			$traite = $montantpret / $nbrtraite;
		    $this->data = $postData;
		}
		/******************************************************/

		if(isset($postData['Affbontraite']['submit']) && isset($postData['Affbontraite'])){
            if($postData['Affbontraite']['agdossier_id']<>'' &&
               $postData['Affbontraite']['date_pret']<>'' &&
               $postData['Affbontraite']['montant_ret']<>'' &&
               $postData['Affbontraite']['montant_traite']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Affbontraite ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Affbontraite->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affbontraites', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Affbontraites', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Affbontraite->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affbontraites', 'view'=>'index', 'params'=>array()),
				'options'=>array('class'=>'toolbarItem back')
			);
		}else{
			$toolbar['Retour'] = array(
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TRAITE MENSUEL':'MODIFICATION TRAITE MENSUEL'));
		$this->set('toolbar', $toolbar);

		$this->set('agdossiers',$this->Agdossier->find('list',array('list'=>array('id','VirtualFields.name')
		, 'fields'=>'id, CONCAT(ag_matricule," ",ag_nom," ",ag_prenom) as name',  'order'=>'Agdossier.ag_nom ASC')));

		$this->set('affbontraites', $this->Affbonretenue->find('list', array('list'=>array('id','montant_ret'), 'order'=>'id ASC')));

		$this->set('statuts', array('1'=>'En cours','2'=>'Echu'));
        $this->set('nbrtraite',$nbrtraite);
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TRAITE MENSUEL'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Affbontraite->find('all', array('conditions'=>array(array($this->Affbontraite->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Affbontraites';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Affbontraite']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Affbontraite->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES AffbontraiteS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Affbontraites', $this->paginate('Affbontraite'));
