...
class AffdotverresController extends AppController{	
	var $paginate = array(
		'Affdotverre'=>array(
			'model'=>'Affdotverre','sort'=>'id', 'affdotverre'=>'ASC',
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Affdotverres'));
		if($accessLevel['view']){
        ...
        /******************************************************/
		if(isset($postData['Affdotverre']['valider'])){
			$annee = $postData['Affdotverre']['annee'];
		    $this->data = $postData;
		}
		/******************************************************/
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOTATION VERRE CORRECTEUR <span class="pageTitle">'.$name . SEP . $username.'</span>');
		 ...
		 $this->paginate['Affdotverre']['conditions'][] = array('Affdotverre.annee'=>$annee);

		$this->set('affdotverres', $this->paginate('Affdotverre'));
	...
	public function edit(){
        ...
        //print_r($agcontrats);
		if(isset($postData['Affdotverre']['submit']) && isset($postData['Affdotverre'])){
            if($postData['Affdotverre']['annee']<>'' &&
               $postData['Affdotverre']['montant']<>'')
            {
					/*********************************************************************/
					...
					foreach ($agcontrats as $agcontrat){
						...
						# check...
						$alpha = $this->Affdotverre->find('all', array('conditions'=>array("Affdotverre.annee='{$postData['Affdotverre']['annee']}'","Affdotverre. 	agdossier_id='{$agcontrat['agdossier_id']}'"), 'recursive'=>0));
						$count = count($alpha);
						...
						{
	                        /*---------------------------------------*/
	                        $agent = array('Affdotverre'=>array(
						        'agdossier_id' => $agcontrat['agdossier_id'],
						        'matricule' => $agcontrat['matricule'],
						        'annee' => $postData['Affdotverre']['annee'],
							    'dotation' => $postData['Affdotverre']['montant'],
							    'dotation_restant' => $postData['Affdotverre']['montant'],
								   )
					            );
							$saveId = $this->Affdotverre->save($agent);
						}
						else
						{
                        	continue;
					...
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Affdotverre ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						//$saveId = $this->Affdotverre->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affdotverres', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Affdotverres', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Affdotverre->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affdotverres', 'view'=>'index', 'params'=>array()),
	...
	public function index2() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOTATION VERRES CORRECTEURS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		 $this->paginate['Affdotverre']['conditions'][] = array('Affdotverre.annee'=>$annee);
		$this->set('affdotverres', $this->paginate('Affdotverre'));
	...
	public function modif(){
		...
		if(isset($postData['Affdotverre']['submit']) && isset($postData['Affdotverre'])){
            if($postData['Affdotverre']['annee']<>'' &&
               $postData['Affdotverre']['matricule']<>'' &&
               $postData['Affdotverre']['dotation']<>'' &&
               $postData['Affdotverre']['dotation_restant']<>'')
            {
	...
	public function traitement() {
        ...
        /*---------------------------------*/
		if(isset($postData['Affdotverre']['valider'])){
			$datedebut = $postData['Affdotverre']['date_debut'];
			$datefin = $postData['Affdotverre']['date_fin'];
			$agdossier = $postData['Affdotverre']['agdossier_id'];
		    $this->data = $postData;
		}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Affdotverre->find('all', array('conditions'=>array(array($this->Affdotverre->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Affdotverres';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Affdotverre']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Affdotverre->delete($ids);
