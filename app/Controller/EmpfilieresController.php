...
class EmpfilieresController extends AppController{	
	var $paginate = array(
		'Empfiliere'=>array(
			'model'=>'Empfiliere','sort'=>'id', 'empfiliere'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Empfilieres'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'FILIERES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('empfilieres', $this->paginate('Empfiliere'));
	...
	public function edit() {
		...
		if(isset($postData['Empfiliere']['submit']) && isset($postData['Empfiliere'])){
            if($postData['Empfiliere']['intitule']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Empfiliere ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Empfiliere->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Empfilieres', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Empfilieres', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Empfiliere->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Empfilieres', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION FILIERE':'MODIFICATION FILIERE'));
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Empfiliere->find('all', array('conditions'=>array(array($this->Empfiliere->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Empfilieres';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Empfiliere']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Empfiliere->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES FILIERES'));
		//$this->set('toolbar', $toolbar);
		$this->set('Empfilieres', $this->paginate('Empfiliere'));
