...
class ParamdirectionsController extends AppController{	
	var $paginate = array(
		'Paramdirection'=>array(
			'model'=>'Paramdirection','sort'=>'nom_direction', 'paramdirection'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramdirections'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'CATEGORIES SOCIO-PROFESSIONNELLES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramdirections', $this->paginate('Paramdirection'));
	...
	public function edit() {
		...
		if(isset($postData['Paramdirection']['submit']) && isset($postData['Paramdirection'])){
            if($postData['Paramdirection']['nom_direction']<>'' &&  $postData['Paramdirection']['sigle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramdirection ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramdirection->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramdirections', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramdirections', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramdirection->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramdirections', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DE DIRECTION':'MODIFICATION DE DIRECTION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DE DIRECTION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramdirection->find('all', array('conditions'=>array(array($this->Paramdirection->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramdirections';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramdirection']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramdirection->delete($ids);
