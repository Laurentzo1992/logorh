...
class CarevalitemsController extends AppController{	
	var $paginate = array(
		'Carevalitem'=>array(
			'model'=>'Carevalitem','sort'=>'id', 'carevalitem'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Carevalitems'));
		if($accessLevel['view']){
       ...
       // pretid
		...
		$this->paginate['Carevalitem']['conditions'][] = array('Carevalitem.affpret_id'=>$pretid);
		$this->set('carevalitems', $this->paginate('Carevalitem'));
	...
	public function edit() {
		...
		$postData = $this->postData();
		 ...
		 //montant_pret
        /******************************************************/
		if(isset($postData['Carevalitem']['valider'])){
			$nbrtraite = $postData['Carevalitem']['nbr_traite'];
			$montantpret = $postData['Carevalitem']['montant_pret'];
			$traite = $montantpret / $nbrtraite;
		    $this->data = $postData;
		}
		/******************************************************/

		if(isset($postData['Carevalitem']['submit']) && isset($postData['Carevalitem'])){
            if($postData['Carevalitem']['agdossier_id']<>'' &&
               $postData['Carevalitem']['date_pret']<>'' &&
               $postData['Carevalitem']['montant_pret']<>'' &&
               $postData['Carevalitem']['montant_traite']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Carevalitem ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Carevalitem->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carevalitems', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carevalitems', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carevalitem->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carevalitems', 'view'=>'index', 'params'=>array()),
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carevalitem->find('all', array('conditions'=>array(array($this->Carevalitem->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carevalitems';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carevalitem']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carevalitem->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES CarevalitemS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Carevalitems', $this->paginate('Carevalitem'));
