...
class CarcriteresController extends AppController{	
	var $paginate = array(
		'Carcritere'=>array(
			'model'=>'Carcritere','sort'=>'id', 'carcritere'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramsociopro','Paramtypefonction','Paramtypevaluation');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Carcriteres'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'CRITERES D\'APPRECIATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('carcriteres', $this->paginate('Carcritere'));
	...
	public function edit() {
		...
		if(isset($postData['Carcritere']['submit']) && isset($postData['Carcritere'])){
            if($postData['Carcritere']['libelle']<>'' &&
               $postData['Carcritere']['paramtypevaluation_id']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Carcritere ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Carcritere->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carcriteres', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carcriteres', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carcritere->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carcriteres', 'view'=>'index', 'params'=>array()),
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carcritere->find('all', array('conditions'=>array(array($this->Carcritere->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carcriteres';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carcritere']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carcritere->delete($ids);
