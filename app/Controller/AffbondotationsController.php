...
class AffbondotationsController extends AppController{	
	var $paginate = array(
		'Affbondotation'=>array(
			'model'=>'Affbondotation','sort'=>'id', 'affbondotation'=>'ASC',
	...
	var $uses = array('User','Agdossier','Parampharmacie','Affbonpharma','Paramregimemedico','Agcontrat','Signataire');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Affbondotations'));
		if($accessLevel['view']){
        ...
        /******************************************************/
		if(isset($postData['Affbondotation']['valider'])){
			$annee = $postData['Affbondotation']['annee'];
		    $this->data = $postData;
		}
		/******************************************************/

		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOTATION PRODUITS PHARMACEUTIQUES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		 $this->paginate['Affbondotation']['conditions'][] = array('Affbondotation.annee'=>$annee);
		$this->set('affbondotations', $this->paginate('Affbondotation'));
	...
	public function edit(){
		...
		if(isset($postData['Affbondotation']['submit']) && isset($postData['Affbondotation'])){
            if($postData['Affbondotation']['annee']<>'' &&
               $postData['Affbondotation']['montant']<>'')
            {
					/*********************************************************************/
	                $agcontrats = $this->Agcontrat->find('all', array('conditions'=>array('Agcontrat.statut=1'), 'recursive'=>0));
					foreach ($agcontrats as $agcontrat){
						$agcontrat = $agcontrat['Agcontrat'];
						# check...
						$alpha = $this->Affbondotation->find('all', array('conditions'=>array("Affbondotation.annee='{$postData['Affbondotation']['annee']}'","Affbondotation. 	agdossier_id='{$agcontrat['agdossier_id']}'"), 'recursive'=>0));
						$count = count($alpha);
						...
						{
	                        $dot_rest_ant = 0;
	                        $year = $postData['Affbondotation']['annee'] - 1;

	                        $beta = $this->Affbondotation->find('all', array('conditions'=>array("Affbondotation.annee='{$year}'","Affbondotation. 	agdossier_id='{$agcontrat['agdossier_id']}'"), 'recursive'=>0));
						    $count2 = count($beta);
                            if($count2 == 1)
						    {
                               $dot_rest_ant = $beta[0]['Affbondotation']['dotation_restant'];
						    }
						    else
						    {
						    	$dot_rest_ant = 0;
	                        ...
	                        /*---------------------------------------*/
	                        $agent = array('Affbondotation'=>array(
						        'agdossier_id' => $agcontrat['agdossier_id'],
						        'matricule' => $agcontrat['matricule'],
						        'annee' => $postData['Affbondotation']['annee'],
							    'dotation' => $postData['Affbondotation']['montant'],
							    'dotation_restant' => $postData['Affbondotation']['montant'] + $dot_rest_ant,
								   )
					            );
							$saveId = $this->Affbondotation->save($agent);
						}
						else
						{
                        	continue;
					...
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Affbondotation ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						//$saveId = $this->Affbondotation->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affbondotations', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Affbondotations', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Affbondotation->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affbondotations', 'view'=>'index', 'params'=>array()),
	...
	public function modif(){
		...
		if(isset($postData['Affbondotation']['submit']) && isset($postData['Affbondotation'])){
            if($postData['Affbondotation']['annee']<>'' &&
               $postData['Affbondotation']['matricule']<>'' &&
               $postData['Affbondotation']['dotation']<>'' &&
               $postData['Affbondotation']['dotation_restant']<>'')
            {
	...
	public function traitement() {
        ...
        /*---------------------------------*/
		if(isset($postData['Affbondotation']['valider'])){
			$datedebut = $postData['Affbondotation']['date_debut'];
			$datefin = $postData['Affbondotation']['date_fin'];
			$agdossier = $postData['Affbondotation']['agdossier_id'];
		    $this->data = $postData;
		}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Affbondotation->find('all', array('conditions'=>array(array($this->Affbondotation->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Affbondotations';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Affbondotation']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Affbondotation->delete($ids);
