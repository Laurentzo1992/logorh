...
class ParamtypemvtsController extends AppController{	
	var $paginate = array(
		'Paramtypemvt'=>array(
			'model'=>'Paramtypemvt','sort'=>'libelle', 'paramtypemvt'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtypemvts'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPES MOUVEMENTS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtypemvts', $this->paginate('Paramtypemvt'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtypemvt']['submit']) && isset($postData['Paramtypemvt'])){
            if($postData['Paramtypemvt']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtypemvt ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtypemvt->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtypemvts', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtypemvts', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtypemvt->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtypemvts', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE MOUVEMENT':'MODIFICATION TYPE MOUVEMENT'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE MOUVEMENT'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtypemvt->find('all', array('conditions'=>array(array($this->Paramtypemvt->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtypemvts';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtypemvt']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtypemvt->delete($ids);
