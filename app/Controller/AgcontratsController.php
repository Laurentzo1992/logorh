...
class AgcontratsController extends AppController{	
	var $paginate = array(
		'Agcontrat'=>array(
	...
	var $uses = array('User','Agdossier','Paramclassification','Paramechelon','Paramnatcontrat',
                      'Paramstructurecotsocial','Paramservice','Paramdirection','Parambanque',
                      'Parammodepaie','Paramfonction','Paramstatactivation','Paramtypemvt');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Agcontrats'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'INFORMATIONS CONTRACTUELS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('agcontrats', $this->paginate('Agcontrat'));
		...
		$this->set('paramservices', $this->Paramservice->find('list', array('list'=>array('id',' 	nom_service'), 'order'=>'id ASC')));

		$this->set('paramdirections', $this->Paramdirection->find('list', array('list'=>array('id',' 	nom_direction'), 'order'=>'id ASC')));
	...
	public function index2() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'AFFECTATION / MUTATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('agcontrats', $this->paginate('Agcontrat'));
	...
	public function index4() {
		...
		$this->paginate['Agcontrat']['conditions'][] = array('Agcontrat.statut'=>'1');
		$this->set('agcontrats', $this->paginate('Agcontrat'));
	...
	public function index5() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'EVALUATION AGENT <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('agcontrats', $this->paginate('Agcontrat'));
	...
	public function index7() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'Valeur - éléments de salaire <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('agcontrats', $this->paginate('Agcontrat'));
	...
	public function edit() {
		...
		if(isset($postData['Agcontrat']['submit']) && isset($postData['Agcontrat'])){
            if($postData['Agcontrat']['agdossier_id']<>'' &&
			   $postData['Agcontrat']['paramnatcontrat_id']<>'' &&
			   $postData['Agcontrat']['date_debut']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Agcontrat ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){
					/*-----------------------------------*/
                    $tmp = $this->Agdossier->find('all', array('conditions'=>array('Agdossier.id='.$postData['Agcontrat']['agdossier_id']), 'recursive'=>0));
		            $matricule = $tmp[0]['Agdossier']['ag_matricule'];
                    /*-----------------------------------*/	
                   // print_r($matricule);
                    $postData['Agcontrat']['matricule']  = $matricule;			
				    $saveId = $this->Agcontrat->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Agcontrats', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Agcontrats', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Agcontrat->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Agcontrats', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION INFORMATION CONTRACTUEL':'MODIFICATION INFORMATION CONTRACTUEL'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE INFORMATION CONTRACTUEL'));
		$this->set('toolbar', $toolbar);
	}
	/**********Affectation/mutation**************************/
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Agcontrat->find('all', array('conditions'=>array(array($this->Agcontrat->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Agcontrats';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Agcontrat']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Agcontrat->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES DOSSIERS AGENTS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Agcontrats', $this->paginate('Agcontrat'));
