...
class ParamtypevaluationsController extends AppController{	
	var $paginate = array(
		'Paramtypevaluation'=>array(
			'model'=>'Paramtypevaluation','sort'=>'id', 'paramtypevaluation'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtypevaluations'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE EVALUATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtypevaluations', $this->paginate('Paramtypevaluation'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtypevaluation']['submit']) && isset($postData['Paramtypevaluation'])){
            if($postData['Paramtypevaluation']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtypevaluation ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtypevaluation->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtypevaluations', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtypevaluations', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtypevaluation->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtypevaluations', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE EVALUATION':'MODIFICATION TYPE EVALUATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE EVALUATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtypevaluation->find('all', array('conditions'=>array(array($this->Paramtypevaluation->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtypevaluations';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtypevaluation']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtypevaluation->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE TYPE EVALUATION'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramtypevaluations', $this->paginate('Paramtypevaluation'));
