...
class ParamfonctionsController extends AppController{	
	var $paginate = array(
		'Paramfonction'=>array(
			'model'=>'Paramfonction','sort'=>'nom_fonction', 'paramfonction'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramfonctions'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'FONCTIONS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramfonctions', $this->paginate('Paramfonction'));
	...
	public function edit() {
		...
		if(isset($postData['Paramfonction']['submit']) && isset($postData['Paramfonction'])){
            if($postData['Paramfonction']['nom_fonction']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramfonction ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramfonction->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramfonctions', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramfonctions', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramfonction->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramfonctions', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION FONCTION':'MODIFICATION FONCTION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE FONCTION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramfonction->find('all', array('conditions'=>array(array($this->Paramfonction->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramfonctions';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramfonction']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramfonction->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES FONCTIONS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramfonctions', $this->paginate('Paramfonction'));
