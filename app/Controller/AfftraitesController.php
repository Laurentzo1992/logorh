...
class AfftraitesController extends AppController{	
	var $paginate = array(
		'Afftraite'=>array(
			'model'=>'Afftraite','sort'=>'id', 'afftraite'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Affpret');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Afftraites'));
		if($accessLevel['view']){
       ...
       // pretid
        ...
        $tmp = $this->Affpret->find('all', array('conditions'=>array('Affpret.id='.$pretid), 'recursive'=>0));
        $id = $tmp[0]['Affpret']['id'];
		$agdossier_id = $tmp[0]['Affpret']['agdossier_id'];
		$montant_pret = $tmp[0]['Affpret']['montant_pret'];
        /********************************************************************/
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TRAITES MENSUELS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->paginate['Afftraite']['conditions'][] = array('Afftraite.affpret_id'=>$pretid);
		$this->set('afftraites', $this->paginate('Afftraite'));
        
        $this->set('toolbar',$toolbar);
        $this->set('pretid',$pretid);
        $this->set('agdossier_id',$agdossier_id);
        $this->set('montant_pret',$montant_pret);
		...
		$this->set('affprets', $this->Affpret->find('list', array('list'=>array('id','montant_pret'), 'order'=>'id ASC')));
	...
	public function edit() {
		...
		$postData = $this->postData();
		 ...
		 //montant_pret
        /******************************************************/
		if(isset($postData['Afftraite']['valider'])){
			$nbrtraite = $postData['Afftraite']['nbr_traite'];
			$montantpret = $postData['Afftraite']['montant_pret'];
			$traite = $montantpret / $nbrtraite;
		    $this->data = $postData;
		}
		/******************************************************/

		if(isset($postData['Afftraite']['submit']) && isset($postData['Afftraite'])){
            if($postData['Afftraite']['agdossier_id']<>'' &&
               $postData['Afftraite']['date_pret']<>'' &&
               $postData['Afftraite']['montant_pret']<>'' &&
               $postData['Afftraite']['montant_traite']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Afftraite ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Afftraite->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Afftraites', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Afftraites', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Afftraite->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Afftraites', 'view'=>'index', 'params'=>array()),
				'options'=>array('class'=>'toolbarItem back')
			);
		}else{
			$toolbar['Retour'] = array(
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Afftraite->find('all', array('conditions'=>array(array($this->Afftraite->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Afftraites';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Afftraite']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Afftraite->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES AfftraiteS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Afftraites', $this->paginate('Afftraite'));
