...
class ParamindemnitesController extends AppController{	
	var $paginate = array(
		'Paramindemnite'=>array(
			'model'=>'Paramindemnite','sort'=>'code', 'paramindemnite'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramavoiret','Paramtypindprimavantage');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramindemnites'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'ELEMENTS SALAIRES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramindemnites', $this->paginate('Paramindemnite'));
	...
	public function edit() {
		...
		if(isset($postData['Paramindemnite']['submit']) && isset($postData['Paramindemnite'])){
            if($postData['Paramindemnite']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramindemnite ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramindemnite->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramindemnites', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramindemnites', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramindemnite->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramindemnites', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION INDEMNITE':'MODIFICATION INDEMNITE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE ELEMENT SALAIRE'));
		$this->set('toolbar', $toolbar);
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramindemnite->find('all', array('conditions'=>array(array($this->Paramindemnite->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramindemnites';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramindemnite']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramindemnite->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES ParamindemniteS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramindemnites', $this->paginate('Paramindemnite'));
