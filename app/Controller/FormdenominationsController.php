...
class FormdenominationsController extends AppController{	
	var $paginate = array(
		'Formdenomination'=>array(
			'model'=>'Formdenomination','sort'=>'id', 'formdenomination'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Formdenominations'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DENOMINATIONS FORMATIONS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('formdenominations', $this->paginate('Formdenomination'));
	...
	public function edit() {
		...
		if(isset($postData['Formdenomination']['submit']) && isset($postData['Formdenomination'])){
             if($postData['Formdenomination']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Formdenomination ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Formdenomination->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Formdenominations', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Formdenominations', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Formdenomination->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Formdenominations', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DENOMINATION FORMATION':'MODIFICATION DENOMINATION FORMATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DENOMINATION FORMATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Formdenomination->find('all', array('conditions'=>array(array($this->Formdenomination->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Formdenominations';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Formdenomination']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Formdenomination->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES DENOMINATION FORMATIONS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Formdenominations', $this->paginate('Formdenomination'));
