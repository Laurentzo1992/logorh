...
class ParamgrillesalsController extends AppController{	
	var $paginate = array(
		'Paramgrillesal'=>array(
			'model'=>'Paramgrillesal','sort'=>'id', 'Paramgrillesal'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramclassification','Paramechelon');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramgrillesals'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'GRILLE SALARIALE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramgrillesals', $this->paginate('Paramgrillesal'));
	...
	public function edit() {
		...
		if(isset($postData['Paramgrillesal']['submit']) && isset($postData['Paramgrillesal'])){
            if($postData['Paramgrillesal']['paramclassification_id']<>'' &&
               $postData['Paramgrillesal']['paramechelon_id']<>'' &&
               $postData['Paramgrillesal']['salaire']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramgrillesal ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramgrillesal->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramgrillesals', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramgrillesals', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramgrillesal->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramgrillesals', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION GRILLE SALARIALE ':'MODIFICATION GRILLE SALARIALE'));
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramgrillesal->find('all', array('conditions'=>array(array($this->Paramgrillesal->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramgrillesals';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramgrillesal']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramgrillesal->delete($ids);
