...
class CarsanctionsController extends AppController{	
	var $paginate = array(
		'Carsanction'=>array(
			'model'=>'Carsanction','sort'=>'id', 'carsanction'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Cartypesanction');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Carsanctions'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SANCTION<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('carsanctions', $this->paginate('Carsanction'));
	...
	public function edit() {
		...
		if(isset($postData['Carsanction']['submit']) && isset($postData['Carsanction'])){
            if($postData['Carsanction']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Carsanction ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Carsanction->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carsanctions', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carsanctions', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carsanction->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carsanctions', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION SANCTION':'MODIFICATION SANCTION'));
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carsanction->find('all', array('conditions'=>array(array($this->Carsanction->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carsanctions';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carsanction']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carsanction->delete($ids);
