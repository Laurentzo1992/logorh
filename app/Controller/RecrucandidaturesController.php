...
class RecrucandidaturesController extends AppController{	
	var $paginate = array(
		'Recrucandidature'=>array(
			'model'=>'Recrucandidature','sort'=>'id', 'recrucandidature'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Recruposte','Recrusession');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Recrucandidatures'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DOSSIERS CANDIDATURES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('recrucandidatures', $this->paginate('Recrucandidature'));
		
		$this->set('recrupostes', $this->Recruposte->find('list', array('list'=>array('id','intitule'), 'order'=>'id ASC')));

		$this->set('matrimoniales', array('1'=>'Célibataire','2'=>'Marié(e)','3'=>'Divorcé(e)','4'=>'Veuf/Veuve'));
	    
	    $this->set('recrusessions', $this->Recrusession->find('list', array('list'=>array('id','annee'), 'order'=>'annee ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Recrucandidature']['submit']) && isset($postData['Recrucandidature'])){
            if($postData['Recrucandidature']['ag_nom']<>'' &&
			   $postData['Recrucandidature']['ag_prenom']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Recrucandidature ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Recrucandidature->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Recrucandidatures', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Recrucandidatures', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Recrucandidature->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Recrucandidatures', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DOSSIER CANDIDATURE':'MODIFICATION DOSSIER CANDIDATURE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DOSSIER CANDIDATURE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Recrucandidature->find('all', array('conditions'=>array(array($this->Recrucandidature->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Recrucandidatures';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Recrucandidature']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Recrucandidature->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES DOSSIERS CANDIDATURES'));
		//$this->set('toolbar', $toolbar);
		$this->set('Recrucandidatures', $this->paginate('Recrucandidature'));
