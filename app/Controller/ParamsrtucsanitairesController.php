...
class ParamsrtucsanitairesController extends AppController{	
	var $paginate = array(
		'Paramsrtucsanitaire'=>array(
			'model'=>'Paramsrtucsanitaire','sort'=>'id', 'Paramsrtucsanitaire'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramsrtucsanitaires'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'STUCTURE SANITAIRE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramsrtucsanitaires', $this->paginate('Paramsrtucsanitaire'));

		 $this->set('types', array('Hopital'=>'Hopital','Clinique'=>'Clinique',));
	...
	public function edit() {
		...
		if(isset($postData['Paramsrtucsanitaire']['submit']) && isset($postData['Paramsrtucsanitaire'])){
            if($postData['Paramsrtucsanitaire']['nom_structure']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramsrtucsanitaire ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramsrtucsanitaire->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramsrtucsanitaires', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramsrtucsanitaires', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramsrtucsanitaire->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramsrtucsanitaires', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION STUCTURE SANITAIRE ':'MODIFICATION STUCTURE SANITAIRE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE STUCTURE SANITAIRE '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramsrtucsanitaire->find('all', array('conditions'=>array(array($this->Paramsrtucsanitaire->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramsrtucsanitaires';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramsrtucsanitaire']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramsrtucsanitaire->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('STUCTURE SANITAIRE'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramsrtucsanitaires', $this->paginate('Paramsrtucsanitaire'));
