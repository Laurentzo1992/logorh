...
class RecrusessionsController extends AppController{	
	var $paginate = array(
		'Recrusession'=>array(
			'model'=>'Recrusession','sort'=>'annee', 'recrusession'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Parambudget');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Recrusessions'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SESSION RECRUTEMENT <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('recrusessions', $this->paginate('Recrusession'));
	...
	public function edit() {
		...
		if(isset($postData['Recrusession']['submit']) && isset($postData['Recrusession'])){
            if($postData['Recrusession']['annee']<>'' &&
			   $postData['Recrusession']['parambudget_id']<>'' &&
			   $postData['Recrusession']['montant']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Recrusession ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Recrusession->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Recrusessions', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Recrusessions', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Recrusession->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Recrusessions', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION SESSION RECRUTEMENT':'MODIFICATION SESSION RECRUTEMENT'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE SESSION RECRUTEMENT'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Recrusession->find('all', array('conditions'=>array(array($this->Recrusession->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Recrusessions';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Recrusession']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Recrusession->delete($ids);
