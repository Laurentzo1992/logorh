...
class ParamavoiretsController extends AppController{	
	var $paginate = array(
		'Paramavoiret'=>array(
			'model'=>'Paramavoiret','sort'=>'libelle', 'paramavoiret'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramavoirets'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'AVOIRS / RETERNUES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramavoirets', $this->paginate('Paramavoiret'));
	...
	public function edit() {
		...
		if(isset($postData['Paramavoiret']['submit']) && isset($postData['Paramavoiret'])){
            if($postData['Paramavoiret']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramavoiret ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramavoiret->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramavoirets', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramavoirets', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramavoiret->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramavoirets', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION AVOIR / RETENUE':'MODIFICATION  AVOIR / RETENUE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE AVANTAGE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramavoiret->find('all', array('conditions'=>array(array($this->Paramavoiret->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramavoirets';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramavoiret']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramavoiret->delete($ids);
