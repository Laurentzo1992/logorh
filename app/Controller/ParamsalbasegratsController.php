...
class ParamsalbasegratsController extends AppController{	
	var $paginate = array(
		'Paramsalbasegrat'=>array(
			'model'=>'Paramsalbasegrat','sort'=>'matricule', 'paramsalbasegrat'=>'ASC',
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramsalbasegrats'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SALAIRE DE BASE GRATIFICATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramsalbasegrats', $this->paginate('Paramsalbasegrat'));
	...
	public function edit() {
		...
		if(isset($postData['Paramsalbasegrat']['submit']) && isset($postData['Paramsalbasegrat'])){
            if($postData['Paramsalbasegrat']['agdossier_id']<>'' &&
			   $postData['Paramsalbasegrat']['salaire_base_grat']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramsalbasegrat ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){
					/*-----------------------------------*/
                    $tmp = $this->Agdossier->find('all', array('conditions'=>array('Agdossier.id='.$postData['Paramsalbasegrat']['agdossier_id']), 'recursive'=>0));
		            $matricule = $tmp[0]['Agdossier']['ag_matricule'];
                    /*-----------------------------------*/	
                   // print_r($matricule);
                    $postData['Paramsalbasegrat']['matricule']  = $matricule;			
				    $saveId = $this->Paramsalbasegrat->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramsalbasegrats', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramsalbasegrats', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramsalbasegrat->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramsalbasegrats', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION SALAIRE DE BASE GRATIFICATION':'MODIFICATION SALAIRE DE BASE GRATIFICATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE SALAIRE DE BASE GRATIFICATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramsalbasegrat->find('all', array('conditions'=>array(array($this->Paramsalbasegrat->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramsalbasegrats';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramsalbasegrat']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramsalbasegrat->delete($ids);
