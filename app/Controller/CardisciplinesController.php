...
class CardisciplinesController extends AppController{	
	var $paginate = array(
		'Cardiscipline'=>array(
			'model'=>'Cardiscipline','sort'=>'id', 'carsanction'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Cartypediscipline');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Cardisciplines'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DISCIPLINE<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('cardisciplines', $this->paginate('Cardiscipline'));
		...
		$this->set('typedisciplines', $this->Cartypediscipline->find('list', array('list'=>array('id','libelle'), 'order'=>'libelle ASC')));
	}
	...
	public function edit() {
		...
		if(isset($postData['Cardiscipline']['submit']) && isset($postData['Cardiscipline'])){
            if($postData['Cardiscipline']['agdossier_id']<>'' &&
			   $postData['Cardiscipline']['cartypediscipline_id']<>'' &&
			   $postData['Cardiscipline']['date_disc']<>'' &&
			   $postData['Cardiscipline']['motif']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Cardiscipline ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Cardiscipline->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Cardisciplines', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Cardisciplines', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Cardiscipline->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Cardisciplines', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DISCIPLINE':'MODIFICATION DISCIPLINE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DISCIPLINE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Cardiscipline->find('all', array('conditions'=>array(array($this->Cardiscipline->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Cardisciplines';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Cardiscipline']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Cardiscipline->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DISCIPLINE'));
		//$this->set('toolbar', $toolbar);
		$this->set('Cardisciplines', $this->paginate('Cardiscipline'));
