...
class TmpointagesController extends AppController{	
	var $paginate = array(
		'Tmpointage'=>array(
			'model'=>'Tmpointage','sort'=>'id', 'tmpointage'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Tmpointages'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'POINTAGE<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('tmpointages', $this->paginate('Tmpointage'));
	...
	public function edit() {
		...
		if(isset($postData['Tmpointage']['submit']) && isset($postData['Tmpointage'])){
            if($postData['Tmpointage']['agdossier_id']<>'' &&
			   $postData['Tmpointage']['date_pt']<>'' &&
			   $postData['Tmpointage']['heure_pt']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Tmpointage ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){
                    ...
                    /*-----------------------------------*/	
                    // print_r($matricule);
                    $postData['Tmpointage']['matricule']  = $matricule;						
					$saveId = $this->Tmpointage->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Tmpointages', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Tmpointages', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Tmpointage->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Tmpointages', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE DISCIPLINE':'MODIFICATION TYPE DISCIPLINE'));
	...
	public function import() {
			...
			$this->requestAction('Logs' ,'record', $log);
			 ...
			 /*=======================IMPORTATION=====================================*/
                ...
                { 
                    ...
                    $CSVResultStat =   $_FILES['fichiercsv']['tmp_name'];
                   // $CSVResultStat =   $_FILES['fichiercsv']['name'];

                        // Si on peut ouvrir le fichier, alors choisir de le lire
                        if (($handle = fopen($CSVResultStat, 'r')) !== FALSE)
                        // Ouverture fichier
                        {
                            $ex_header = true;
                            $import = false;
                            WHILE(($column = fgetcsv($handle, 2500, ';', '"')) !== FALSE)
                            {
	                               
                                //Exclure les entetes (1ere ligne)
							    if(!$ex_header)
							    {
	                               ...
	                               $gdor = array();
	                               $participants = $this->Tmpointage->find('all', 
	                               	    array('conditions'=>array(
	                   	    	                   "Tmpointage.date_pt='".$column[0]."'"
                           	    	         ), 
	                                        'recursive'=>0));
	                               ...
	                               if($nb > 0)
	                                {
	                                    ...
	                                    $participantID = $participants[0]['Tmpointage']['id'];
	                                    $gdor = array('Tmpointage'=>array(
		                                    	        'id'=>$participantID,
		                                    	        'date_pt'=>$column[0],
                           	    	                    'heure_pt'=>$column[1],
                           	    	                    'agdossier_id'=>$column[2],
                           	    	                    'matricule'=>$column[3]
                           	    	                   
                           	    	                )
			                                    );

							            $this->Tmpointage->save($gdor);
	                                    $import = true;   
							            //$msg .= $nb.' MISE A JOUR <br>';
		                            ...
		                            else
		                            {
							            ...
							            $this->Tmpointage->save($gdor);
							            $import = true;
							            //$msg .= $nb.' ENREGISTREMENT <br>';
		                            }
		                        }else $ex_header=false;    
	                            
                            }
                            if($import == true)
                            {
                               $this->Session->setFlash('Fichier pointage importé avec succès', 'flash success');
                            }
                            else
                            {
                               $this->Session->setFlash('Fichier pointage non importée', 'flash error');
                            }                      
                        }
                        else  
                        {
                        	$msg = 'Erreur ouverture fichier';
                        }
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'IMPORTATION FICHIER POINTAGE':'IMPORTATION FICHIER POINTAGE'));
		$this->set('toolbar', $toolbar);	
		$this->set('msg',$msg);
	}
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE DISCIPLINE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Tmpointage->find('all', array('conditions'=>array(array($this->Tmpointage->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Tmpointages';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Tmpointage']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Tmpointage->delete($ids);
