...
class RecrubesoinsController extends AppController{	
	var $paginate = array(
		'Recrubesoin'=>array(
			'model'=>'Recrubesoin','sort'=>'id', 'recrubesoin'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Paramdirection');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Recrubesoins'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'EXIGENCE DU POSTE<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('recrubesoins', $this->paginate('Recrubesoin'));
		...
		$this->set('statuts', array('1'=>'En attente','2'=>'Non validé','3'=>'Validé'));
	...
	public function index2() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'EXIGENCE DU POSTE<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->paginate['Recrubesoin']['conditions'][] = array('Recrubesoin.statut'=>3);
		$this->set('recrubesoins', $this->paginate('Recrubesoin'));
	...
	public function edit() {
		...
		if(isset($postData['Recrubesoin']['submit']) && isset($postData['Recrubesoin'])){
            if($postData['Recrubesoin']['intitule']<>'' &&
               $postData['Recrubesoin']['nombre']<>'' &&
               $postData['Recrubesoin']['mission']<>'' &&
               $postData['Recrubesoin']['profil']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Recrubesoin ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Recrubesoin->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Recrubesoins', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Recrubesoins', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Recrubesoin->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Recrubesoins', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION EXIGENCE DU POSTE':'MODIFICATION EXIGENCE DU POSTE'));
	...
	public function edit2() {
		...
		$postData = $this->postData();
		if(isset($postData['Recrubesoin']['submit']) && isset($postData['Recrubesoin'])){
            if($postData['Recrubesoin']['statut']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Recrubesoin ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE EXIGENCE DU POSTE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Recrubesoin->find('all', array('conditions'=>array(array($this->Recrubesoin->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Recrubesoins';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Recrubesoin']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Recrubesoin->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES BESOINS DE RECRUTEMENT'));
		//$this->set('toolbar', $toolbar);
		$this->set('Recrubesoins', $this->paginate('Recrubesoin'));
