...
class ParambudgetsController extends AppController{	
	var $paginate = array(
		'Parambudget'=>array(
			'model'=>'Parambudget','sort'=>'budget', 'parambudget'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Parambudgets'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'BUDGETS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('parambudgets', $this->paginate('Parambudget'));
	...
	public function edit() {
		...
		if(isset($postData['Parambudget']['submit']) && isset($postData['Parambudget'])){
            if($postData['Parambudget']['budget']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Parambudget ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Parambudget->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Parambudgets', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Parambudgets', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Parambudget->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Parambudgets', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION BUDGET':'MODIFICATION BUDGET'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE BUDGET'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Parambudget->find('all', array('conditions'=>array(array($this->Parambudget->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Parambudgets';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Parambudget']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Parambudget->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES BUDGETS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Parambudgets', $this->paginate('Parambudget'));
