...
class ParamservicesController extends AppController{	
	var $paginate = array(
		'Paramservice'=>array(
			'model'=>'Paramservice','sort'=>'id', 'Paramservice'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramdirection','Paramechelon');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramservices'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SERVICES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramservices', $this->paginate('Paramservice'));
	...
	public function edit() {
		...
		if(isset($postData['Paramservice']['submit']) && isset($postData['Paramservice'])){
            if($postData['Paramservice']['paramdirection_id']<>'' &&
               $postData['Paramservice']['nom_service']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramservice ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramservice->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramservices', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramservices', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramservice->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramservices', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION SERVICE ':'MODIFICATION SERVICE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE SERVICE '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramservice->find('all', array('conditions'=>array(array($this->Paramservice->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramservices';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramservice']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramservice->delete($ids);
