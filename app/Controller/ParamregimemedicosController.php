...
class ParamregimemedicosController extends AppController{	
	var $paginate = array(
		'Paramregimemedico'=>array(
			'model'=>'Paramregimemedico','sort'=>'libelle', 'paramregimemedico'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramregimemedicos'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'REGIMES MEDICO-SOCIAL <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramregimemedicos', $this->paginate('Paramregimemedico'));
	...
	public function edit() {
		...
		if(isset($postData['Paramregimemedico']['submit']) && isset($postData['Paramregimemedico'])){
            if($postData['Paramregimemedico']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramregimemedico ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramregimemedico->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramregimemedicos', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramregimemedicos', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramregimemedico->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramregimemedicos', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION REGIME MEDICO-SOCIAL':'MODIFICATION REGIME MEDICO-SOCIAL'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE REGIME MEDICO-SOCIAL'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramregimemedico->find('all', array('conditions'=>array(array($this->Paramregimemedico->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramregimemedicos';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramregimemedico']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramregimemedico->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES REGIMES MEDICO-SOCIAL'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramregimemedicos', $this->paginate('Paramregimemedico'));
