...
class ParamsocioprosController extends AppController{	
	var $paginate = array(
		'Paramsociopro'=>array(
			'model'=>'Paramsociopro','sort'=>'libelle', 'paramsociopro'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramsociopros'));
		if($accessLevel['view']){
		...
		$this->set('paramsociopros', $this->paginate('Paramsociopro'));
	...
	public function edit() {
		...
		if(isset($postData['Paramsociopro']['submit']) && isset($postData['Paramsociopro'])){
            if($postData['Paramsociopro']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramsociopro ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramsociopro->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramsociopros', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramsociopros', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramsociopro->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramsociopros', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION CATEGORIE SOCIO-PROFESSIONNELLE':'MODIFICATION CATEGORIE SOCIO-PROFESSIONNELLE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE CATEGORIE SOCIO-PROFESSIONNELLE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramsociopro->find('all', array('conditions'=>array(array($this->Paramsociopro->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramsociopros';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramsociopro']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramsociopro->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES ParamsocioproS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramsociopros', $this->paginate('Paramsociopro'));
