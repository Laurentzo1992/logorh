...
class ParamtypesalairesController extends AppController{	
	var $paginate = array(
		'Paramtypesalaire'=>array(
			'model'=>'Paramtypesalaire','sort'=>'libelle', 'paramtypesalaire'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramclassification','Agavencement','Agdossier');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtypesalaires'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPES SALAIRES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtypesalaires', $this->paginate('Paramtypesalaire'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtypesalaire']['submit']) && isset($postData['Paramtypesalaire'])){
            if($postData['Paramtypesalaire']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtypesalaire ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtypesalaire->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtypesalaires', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtypesalaires', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtypesalaire->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtypesalaires', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE SALAIRE':'MODIFICATION TYPE SALAIRE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE SALAIRE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtypesalaire->find('all', array('conditions'=>array(array($this->Paramtypesalaire->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtypesalaires';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtypesalaire']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtypesalaire->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES ParamtypesalaireS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramtypesalaires', $this->paginate('Paramtypesalaire'));
