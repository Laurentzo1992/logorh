...
class AffavancesController extends AppController{	
	var $paginate = array(
		'Affavance'=>array(
			'model'=>'Affavance','sort'=>'id', 'affavance'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Affavances'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'AVANCES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('affavances', $this->paginate('Affavance'));
	...
	public function edit() {
		...
		if(isset($postData['Affavance']['submit']) && isset($postData['Affavance'])){
            if($postData['Affavance']['agdossier_id']<>'' &&
               $postData['Affavance']['date_av']<>'' &&
               $postData['Affavance']['montant_av']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Affavance ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Affavance->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Affavances', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Affavances', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Affavance->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Affavances', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION AVANCE':'MODIFICATION AVANCE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE AVANCE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Affavance->find('all', array('conditions'=>array(array($this->Affavance->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Affavances';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Affavance']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Affavance->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES AffavanceS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Affavances', $this->paginate('Affavance'));
