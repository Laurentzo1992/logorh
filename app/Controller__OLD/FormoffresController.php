...
class FormoffresController extends AppController{	
	var $paginate = array(
		'Formoffre'=>array(
			'model'=>'Formoffre','sort'=>'id', 'formoffre'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Formoffres'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'OFFRE DE FORMATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('formoffres', $this->paginate('Formoffre'));

		$this->set('formations', $this->Formation->find('list', array('list'=>array('id','theme'), 'order'=>'theme ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Formoffre']['submit']) && isset($postData['Formoffre'])){
            if($postData['Formoffre']['type']<>'' &&
			   $postData['Formoffre']['formation_id']<>'' &&
			   $postData['Formoffre']['montant']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Formoffre ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Formoffre->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Formoffres', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Formoffres', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Formoffre->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Formoffres', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION OFFRE DE FORMATION':'MODIFICATION OFFRE DE FORMATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE OFFRE DE FORMATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Formoffre->find('all', array('conditions'=>array(array($this->Formoffre->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Formoffres';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Formoffre']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Formoffre->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE OFFRE DE FORMATION'));
		//$this->set('toolbar', $toolbar);
		$this->set('Formoffres', $this->paginate('Formoffre'));
