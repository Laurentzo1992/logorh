...
class ParamechelonsController extends AppController{	
	var $paginate = array(
		'Paramechelon'=>array(
			'model'=>'Paramechelon','sort'=>'id', 'paramechelon'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramechelons'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'ECHELONS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramechelons', $this->paginate('Paramechelon'));
	...
	public function edit() {
		...
		if(isset($postData['Paramechelon']['submit']) && isset($postData['Paramechelon'])){
            if($postData['Paramechelon']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramechelon ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramechelon->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramechelons', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramechelons', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramechelon->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramechelons', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION ECHELON':'MODIFICATION ECHELON'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE ECHELON'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramechelon->find('all', array('conditions'=>array(array($this->Paramechelon->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramechelons';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramechelon']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramechelon->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES ECHELONS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramechelons', $this->paginate('Paramechelon'));
