...
class CarnotesController extends AppController{	
	var $paginate = array(
		'Carnote'=>array(
			'model'=>'Carnote','sort'=>'id', 'carnote'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agcontrat');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Carnotes'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'NOTE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		
		$this->set('carnotes', $this->paginate('Carnote'));
	}
	...
	public function edit() {
		...
		if(isset($postData['Carnote']['submit']) && isset($postData['Carnote'])){
            if($postData['Carnote']['note_min']<>'' &&
               $postData['Carnote']['note_max']<>'' &&
               $postData['Carnote']['appreciation']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Carnote ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Carnote->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carnotes', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carnotes', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carnote->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carnotes', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION NOTE':'MODIFICATION NOTE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE NOTE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carnote->find('all', array('conditions'=>array(array($this->Carnote->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carnotes';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carnote']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carnote->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES NOTES'));
		//$this->set('toolbar', $toolbar);
		$this->set('Carnotes', $this->paginate('Carnote'));
