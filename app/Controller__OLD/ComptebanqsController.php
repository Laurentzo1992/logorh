...
class ComptebanqsController extends AppController{	
	var $paginate = array(
		'Comptebanq'=>array(
			'model'=>'Comptebanq','sort'=>'id', 'parambanque'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Parambanque');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Comptebanqs'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'COMPTES BANQUAIRES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('comptebanqs', $this->paginate('Comptebanq'));
	...
	public function edit() {
		...
		if(isset($postData['Comptebanq']['submit']) && isset($postData['Comptebanq'])){
            if($postData['Comptebanq']['parambanque_id']<>'' &&
			   $postData['Comptebanq']['numcompte']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Comptebanq ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Comptebanq->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Comptebanqs', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Comptebanqs', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Comptebanq->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Comptebanqs', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION COMPTE  BANQUAIRE':'MODIFICATION COMPTE  BANQUAIRE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE COMPTE  BANQUAIRE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Comptebanq->find('all', array('conditions'=>array(array($this->Comptebanq->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Comptebanqs';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Comptebanq']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Comptebanq->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES COMPTES BANQUAIRES'));
		//$this->set('toolbar', $toolbar);
		$this->set('Comptebanqs', $this->paginate('Comptebanq'));
