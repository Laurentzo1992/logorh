...
class CartypedisciplinesController extends AppController{	
	var $paginate = array(
		'Cartypediscipline'=>array(
			'model'=>'Cartypediscipline','sort'=>'id', 'cartypediscipline'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Cartypedisciplines'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE DISCIPLINE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('cartypedisciplines', $this->paginate('Cartypediscipline'));
	}
	...
	public function edit() {
		...
		if(isset($postData['Cartypediscipline']['submit']) && isset($postData['Cartypediscipline'])){
            if($postData['Cartypediscipline']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Cartypediscipline ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Cartypediscipline->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Cartypedisciplines', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Cartypedisciplines', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Cartypediscipline->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Cartypedisciplines', 'view'=>'index', 'params'=>array()),
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Cartypediscipline->find('all', array('conditions'=>array(array($this->Cartypediscipline->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Cartypedisciplines';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Cartypediscipline']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Cartypediscipline->delete($ids);
