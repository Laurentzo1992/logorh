...
class ParamtypefonctionsController extends AppController{	
	var $paginate = array(
		'Paramtypefonction'=>array(
			'model'=>'Paramtypefonction','sort'=>'id', 'paramtypefonction'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramtypevaluation');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtypefonctions'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE FONCTION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtypefonctions', $this->paginate('Paramtypefonction'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtypefonction']['submit']) && isset($postData['Paramtypefonction'])){
            if($postData['Paramtypefonction']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtypefonction ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtypefonction->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtypefonctions', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtypefonctions', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtypefonction->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtypefonctions', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE FONCTION':'MODIFICATION  TYPE FONCTION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE FONCTION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtypefonction->find('all', array('conditions'=>array(array($this->Paramtypefonction->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtypefonctions';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtypefonction']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtypefonction->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE TYPE FONCTION'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramtypefonctions', $this->paginate('Paramtypefonction'));
