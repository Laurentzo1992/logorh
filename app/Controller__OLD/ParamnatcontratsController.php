...
class ParamnatcontratsController extends AppController{	
	var $paginate = array(
		'Paramnatcontrat'=>array(
			'model'=>'Paramnatcontrat','sort'=>'id', 'paramnatcontrat'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramnatcontrats'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'NATURES CONTRATS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramnatcontrats', $this->paginate('Paramnatcontrat'));
	...
	public function edit() {
		...
		if(isset($postData['Paramnatcontrat']['submit']) && isset($postData['Paramnatcontrat'])){
            if($postData['Paramnatcontrat']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramnatcontrat ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramnatcontrat->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramnatcontrats', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramnatcontrats', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramnatcontrat->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramnatcontrats', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION NATURE CONTRAT':'MODIFICATION NATURE CONTRAT'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE NATURE CONTRAT'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramnatcontrat->find('all', array('conditions'=>array(array($this->Paramnatcontrat->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramnatcontrats';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramnatcontrat']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramnatcontrat->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES NATURES CONTRATS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramnatcontrats', $this->paginate('Paramnatcontrat'));
