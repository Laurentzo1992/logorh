...
class CarretraitesController extends AppController{	
	var $paginate = array(
		'Carretraite'=>array(
			'model'=>'Carretraite','sort'=>'id', 'direction'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Agdossier','Agcontrat','Agavencement','Paramsociopro','Paramclassification');
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Carretraites'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'ALERTE(S) DEPART A LA RETRAITE / RUPTURE DE CONTRAT<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('carretraites', $this->paginate('Carretraite'));
	...
	public function index2() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'DEPART A LA RETRAITE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('carretraites', $this->paginate('Carretraite'));
		$this->set('agdossiers',$this->Agdossier->find('list',array('list'=>array('id','VirtualFields.name')
		, 'fields'=>'id, CONCAT(ag_nom," ",ag_prenom) as name',  'order'=>'Agdossier.ag_nom ASC')));
	...
	public function edit() {
		...
		$agdossierID = $this->getGetParam('agdossierID');
		$matricule = $this->getGetParam('matricule');
		$nom = $this->getGetParam('nom');
		$prenom = $this->getGetParam('prenom');
		
		$postData = $this->postData();
        
		if(isset($postData['Carretraite']['submit']) && isset($postData['Carretraite'])){
            if($postData['Carretraite']['agdossier_id']<>'' &&
               $postData['Carretraite']['date_retraite']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' carretraite ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						/*-----------------------------------*/
	                    $tmp = $this->Agdossier->find('all', array('conditions'=>array('Agdossier.id='.$postData['Carretraite']['agdossier_id']), 'recursive'=>0));
			            $matricule = $tmp[0]['Agdossier']['ag_matricule'];
	                    /*-----------------------------------*/	
	                  
	                   $postData['Carretraite']['matricule']  = $matricule;

						$saveId = $this->Carretraite->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carretraites', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carretraites', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carretraite->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carretraites', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DEPART A LA RETRAITE':'MODIFICATION DEPART A LA RETRAITE'));
		...
		$this->set('agdossierID', $agdossierID);
		$this->set('matrricule', $matricule);
		$this->set('nom', $nom);
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carretraite->find('all', array('conditions'=>array(array($this->Carretraite->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carretraites';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carretraite']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carretraite->delete($ids);
