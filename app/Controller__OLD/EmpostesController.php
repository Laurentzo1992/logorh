...
class EmpostesController extends AppController{	
	var $paginate = array(
		'Emposte'=>array(
			'model'=>'Emposte','sort'=>'id', 'emposte'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Employe');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Empostes'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'POSTES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('empostes', $this->paginate('Emposte'));

		$this->set('employes',$this->Employe->find('list',array('list'=>array('id','intitule'),            'order'=>'Employe.intitule ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Emposte']['submit']) && isset($postData['Emposte'])){
            if($postData['Emposte']['intitule']<>'' &&
               $postData['Emposte']['employe_id']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Emposte ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Emposte->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Empostes', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Empostes', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Emposte->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Empostes', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION POSTE':'MODIFICATION POSTE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE POSTE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Emposte->find('all', array('conditions'=>array(array($this->Emposte->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Empostes';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Emposte']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Emposte->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES EmposteS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Empostes', $this->paginate('Emposte'));
