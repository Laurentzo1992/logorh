...
class EmployesController extends AppController{	
	var $paginate = array(
		'Employe'=>array(
			'model'=>'Employe','sort'=>'id', 'employe'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Empfiliere');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Employes'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'EMPLOYES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('employes', $this->paginate('Employe'));

		$this->set('empfilieres',$this->Empfiliere->find('list',array('list'=>array('id','intitule'))));
	...
	public function edit() {
		...
		if(isset($postData['Employe']['submit']) && isset($postData['Employe'])){
            if($postData['Employe']['intitule']<>'' &&
               $postData['Employe']['empfiliere_id']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Employe ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Employe->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Employes', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Employes', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Employe->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Employes', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION EMPLOYE':'MODIFICATION EMPLOYE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE EMPLOYE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Employe->find('all', array('conditions'=>array(array($this->Employe->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Employes';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Employe']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Employe->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES EmployeS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Employes', $this->paginate('Employe'));
