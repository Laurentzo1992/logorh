...
class FormationsController extends AppController{	
	var $paginate = array(
		'Formation'=>array(
			'model'=>'Formation','sort'=>'id', 'formation'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Formdenomination','Parambudget');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Formations'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'FORMATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('formations', $this->paginate('Formation'));

	    $this->set('denominations', $this->Formdenomination->find('list', array('list'=>array('id','libelle'), 'order'=>'id ASC')));

	    $this->set('budgets', $this->Parambudget->find('list', array('list'=>array('id','budget'), 'order'=>'budget ASC')));

	    $this->set('statuts', array('Non planifié'=>'Non planifié','Planifié'=>'Planifié','Exécutée'=>'Exécutée'));
	...
	public function index2() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'CHARGES DE FORMATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('formations', $this->paginate('Formation'));
	...
	public function edit() {
		...
		if(isset($postData['Formation']['submit']) && isset($postData['Formation'])){
             if($postData['Formation']['theme']<>'' &&
                $postData['Formation']['objectif']<>'' &&
                $postData['Formation']['poste']<>'' &&
                $postData['Formation']['nbr_participant']<>'' &&
                $postData['Formation']['modalite']<>'' &&
                $postData['Formation']['duree']<>'' &&
                $postData['Formation']['lieu']<>'' &&
                $postData['Formation']['annee']<>'' &&
                $postData['Formation']['cout']<>'')
            {


					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Formation ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Formation->save($postData);
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Formation->read($id);
			}
		}
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION FORMATION':'MODIFICATION FORMATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE FORMATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function search2() {
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Formations', 'view'=>'index2', 'params'=>array()),
				'options'=>array('class'=>'toolbarItem back')
			);
		}else{
			$toolbar['Retour'] = array(
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Formation->find('all', array('conditions'=>array(array($this->Formation->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Formations';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Formation']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Formation->delete($ids);
    ...
    public function etat() {
        ...
        /*++++++++++++++++++++++++++++++++++++++*/
		if(isset($postData['Formation']['submit'])){
			
			$budget = $postData['Formation']['budget'];
			$denomination = $postData['Formation']['denomination'];
			$dateDebut = date("Y",strtotime($postData['Formation']['dateDebut']));
			$dateFin = date("Y",strtotime($postData['Formation']['dateFin']));
			$this->data = $postData;

		}
		if(isset($postData['Formation']['reinit']))$postData = array();
	     $this->data[0]=$postData;


	       /******************/
	    $curDate = date("Y-m-d");
        $formations = $this->Formation->find('list', array(
								'list'=>array('Formation.id',
								              'Formation.formdenomination_id',
								              'Formation.theme',
								              'Formation.objectif',
								              'Formation.poste',
								              ...
								              'Formation.lieu',
								              'Formation.annee',
								              'Formation.parambudget_id',
								              'Formation.cout',
								              'Formation.statut',
								            ),
								'fields'=>array('Formation.id',
								              'Formation.formdenomination_id',
								              'Formation.theme',
								              'Formation.objectif',
								              'Formation.poste',
								  ...
								  'conditions'=>array(
									"Formation.annee BETWEEN '".$dateDebut."' AND '".$dateFin."' OR
									 Formation.formdenomination_id = '".$denomination."' OR
									 Formation.parambudget_id = '".$budget."'"

							    ),
								/*'conditions'=>array(
									"Participant.pa_matricule LIKE '%{$matricule}%' AND 
									 Participant.pa_nom LIKE '%{$nom}%' AND 
									 Participant.pa_prenom  LIKE '%{$prenom}%' AND
									 Sessionformation.theme_id  LIKE '%{$theme}%' AND  
									 Sessionformation.date_debut  BETWEEN '%{$dateDebut}%' AND '%{$dateFin}%'" 
							    ),*/
							    'order'=>'Formation.formdenomination_id DESC',
       ...
       $w = 'Etats des formations';
	   $this->set('pageTitle', $w);
	   /*$this->set('sessionformations',$Sessionformations);
      ...
      $this->set('denominations', $this->Formdenomination->find('list', array('list'=>array('id', 'libelle'), 'order'=>'Formdenomination.libelle ASC')));

      $this->set('budgets', $this->Parambudget->find('list', array('list'=>array('id','budget'), 'order'=>'Parambudget.budget ASC')));

       $this->set('statuts', array('Plannifié'=>'Plannifié','Exécutée'=>'Exécutée'));
    ...
    public function etat2() {
       ...
       $postData = $this->postData();
		...
		if(isset($postData['Formation']['submit'])){
			...
			$dateDebut = date("Y-m-d",strtotime($postData['Formation']['dateDebut']));
			$dateFin = date("Y-m-d",strtotime($postData['Formation']['dateFin']));
			$this->data = $postData;
