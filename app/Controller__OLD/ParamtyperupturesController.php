...
class ParamtyperupturesController extends AppController{	
	var $paginate = array(
		'Paramtyperupture'=>array(
			'model'=>'Paramtyperupture','sort'=>'id', 'paramtyperupture'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtyperuptures'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE DE RUPTURE<span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtyperuptures', $this->paginate('Paramtyperupture'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtyperupture']['submit']) && isset($postData['Paramtyperupture'])){
            if($postData['Paramtyperupture']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtyperupture ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtyperupture->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtyperuptures', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtyperuptures', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtyperupture->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtyperuptures', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE DE RUPTURE':'MODIFICATION TYPE DE RUPTURE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE DE RUPTURE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtyperupture->find('all', array('conditions'=>array(array($this->Paramtyperupture->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtyperuptures';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtyperupture']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtyperupture->delete($ids);
