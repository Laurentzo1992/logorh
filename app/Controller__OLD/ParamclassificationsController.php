...
class ParamclassificationsController extends AppController{	
	var $paginate = array(
		'Paramclassification'=>array(
			'model'=>'Paramclassification','sort'=>'id', 'paramclassification'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramsociopro');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramclassifications'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'CLASSIFICATIONS <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramclassifications', $this->paginate('Paramclassification'));
	...
	public function edit() {
		...
		if(isset($postData['Paramclassification']['submit']) && isset($postData['Paramclassification'])){
            if($postData['Paramclassification']['libelle']<>'' && $postData['Paramclassification']['code']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramclassification ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramclassification->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramclassifications', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramclassifications', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramclassification->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramclassifications', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION CLASSIFICATION':'MODIFICATION CLASSIFICATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE CLASSIFICATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramclassification->find('all', array('conditions'=>array(array($this->Paramclassification->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramclassifications';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramclassification']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramclassification->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES CLASSIFICATIONS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramclassifications', $this->paginate('Paramclassification'));
