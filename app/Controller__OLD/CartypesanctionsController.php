...
class CartypesanctionsController extends AppController{	
	var $paginate = array(
		'Cartypesanction'=>array(
			'model'=>'Cartypesanction','sort'=>'id', 'cartypesanction'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Cartypesanctions'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE SANCTION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('cartypesanctions', $this->paginate('Cartypesanction'));
	}
	...
	public function edit() {
		...
		if(isset($postData['Cartypesanction']['submit']) && isset($postData['Cartypesanction'])){
            if($postData['Cartypesanction']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Cartypesanction ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Cartypesanction->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Cartypesanctions', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Cartypesanctions', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Cartypesanction->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Cartypesanctions', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE SANCTION':'MODIFICATION TYPE SANCTION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE SANCTION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Cartypesanction->find('all', array('conditions'=>array(array($this->Cartypesanction->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Cartypesanctions';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Cartypesanction']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Cartypesanction->delete($ids);
