...
class ParamstructurecotsocialsController extends AppController{	
	var $paginate = array(
		'Paramstructurecotsocial'=>array(
			'model'=>'Paramstructurecotsocial','sort'=>'nom_structure', 'paramstructurecotsocial'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramstructurecotsocials'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'STRUCTURE COTISATION SOCIALE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramstructurecotsocials', $this->paginate('Paramstructurecotsocial'));
	...
	public function edit() {
		...
		if(isset($postData['Paramstructurecotsocial']['submit']) && isset($postData['Paramstructurecotsocial'])){
            if($postData['Paramstructurecotsocial']['nom_structure']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramstructurecotsocial ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramstructurecotsocial->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramstructurecotsocials', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramstructurecotsocials', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramstructurecotsocial->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramstructurecotsocials', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION DE STRUCTURTE DE COTISATION SOCIALE':'MODIFICATION DE STRUCTURTE DE COTISATION SOCIALE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE DE STRUCTURTE DE COTISATION SOCIALE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramstructurecotsocial->find('all', array('conditions'=>array(array($this->Paramstructurecotsocial->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramstructurecotsocials';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramstructurecotsocial']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramstructurecotsocial->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES ParamstructurecotsocialS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramstructurecotsocials', $this->paginate('Paramstructurecotsocial'));
