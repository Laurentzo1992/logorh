...
class ParamtypindprimavantagesController extends AppController{	
	var $paginate = array(
		'Paramtypindprimavantage'=>array(
			'model'=>'Paramtypindprimavantage','sort'=>'libelle', 'Paramtypindprimavantage'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramclassification');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtypindprimavantages'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPES ELEMENTS DE SALAIRE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtypindprimavantages', $this->paginate('Paramtypindprimavantage'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtypindprimavantage']['submit']) && isset($postData['Paramtypindprimavantage'])){
            if($postData['Paramtypindprimavantage']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Paramtypindprimavantage ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Paramtypindprimavantage->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Paramtypindprimavantages', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Paramtypindprimavantages', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtypindprimavantage->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtypindprimavantages', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE ELEMENT DE SALAIRE ':'MODIFICATION TYPE ELEMENT DE SALAIRE '));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE ELEMENT DE SALAIRE '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtypindprimavantage->find('all', array('conditions'=>array(array($this->Paramtypindprimavantage->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtypindprimavantages';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtypindprimavantage']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtypindprimavantage->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES TYPES ELEMENT DE SALAIRE '));
		//$this->set('toolbar', $toolbar);
		$this->set('Paramtypindprimavantages', $this->paginate('Paramtypindprimavantage'));
