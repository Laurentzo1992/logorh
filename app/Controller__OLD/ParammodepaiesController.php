...
class ParammodepaiesController extends AppController{	
	var $paginate = array(
		'Parammodepaie'=>array(
			'model'=>'Parammodepaie','sort'=>'id', 'Parammodepaie'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Parammodepaies'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'MODE DE PAIEMENT <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('parammodepaies', $this->paginate('Parammodepaie'));
	...
	public function edit() {
		...
		if(isset($postData['Parammodepaie']['submit']) && isset($postData['Parammodepaie'])){
            if($postData['Parammodepaie']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Parammodepaie ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Parammodepaie->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Parammodepaies', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Parammodepaies', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Parammodepaie->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Parammodepaies', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION MODE DE PAIEMENT ':'MODIFICATION MODE DE PAIEMENT'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE MODE DE PAIEMENT '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Parammodepaie->find('all', array('conditions'=>array(array($this->Parammodepaie->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Parammodepaies';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Parammodepaie']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Parammodepaie->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('MODE DE PAIEMENT'));
		//$this->set('toolbar', $toolbar);
		$this->set('Parammodepaies', $this->paginate('Parammodepaie'));
