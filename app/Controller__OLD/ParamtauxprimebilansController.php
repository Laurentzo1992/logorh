...
class ParamtauxprimebilansController extends AppController{	
	var $paginate = array(
		'Paramtauxprimebilan'=>array(
			'model'=>'Paramtauxprimebilan','sort'=>'id', 'direction'=>'ASC',
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Paramtauxprimebilans'));
		if($accessLevel['view']){
	    ...
	    /******************************************************************/
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TAUX PRIME DE BILAN <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('paramtauxprimebilans', $this->paginate('Paramtauxprimebilan'));
	...
	public function edit() {
		...
		if(isset($postData['Paramtauxprimebilan']['submit']) && isset($postData['Paramtauxprimebilan'])){
			$log = ($this->getGetParam('id')?'Modification':'Creation') . ' paramtauxprimebilan ' . 'id: ';
			if($postData['Paramtauxprimebilan']['taux']<>'')
            {
				
				/*********************************************************/
				if($accessLevel['view'] && $accessLevel['edit']){				
					$saveId = $this->Paramtauxprimebilan->save($postData);
					if($saveId){
						$log .= $saveId;
						...
						if($this->Session->check('return')){
							...
							$this->redirect(array('controller'=>'Paramtauxprimebilans', 'view'=>'index'));
						}
					}else {
					//Display Errors
					}
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Paramtauxprimebilan->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Paramtauxprimebilans', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TAUX PRIME DE BILAN':'MODIFICATION TAUX PRIME DE BILAN'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TAUX PRIME DE BILAN'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Paramtauxprimebilan->find('all', array('conditions'=>array(array($this->Paramtauxprimebilan->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Paramtauxprimebilans';
			$dataList = array();
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Paramtauxprimebilan']['id'];
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Paramtauxprimebilan->delete($ids);
    ...
    public function etat() {
		...
		$this->set('pageTitle', ('LISTE TAUX PRIME DE BILAN'));
		$this->set('paramtauxprimebilans', $this->paginate('Paramtauxprimebilan'));
    }
