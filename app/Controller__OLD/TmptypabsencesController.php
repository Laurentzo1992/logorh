...
class TmptypabsencesController extends AppController{	
	var $paginate = array(
		'Tmptypabsence'=>array(
			'model'=>'Tmptypabsence','sort'=>'id', 'tmptypabsence'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Tmptypabsences'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'TYPE ABSENCE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('tmptypabsences', $this->paginate('Tmptypabsence'));
	}
	...
	public function edit() {
		...
		if(isset($postData['Tmptypabsence']['submit']) && isset($postData['Tmptypabsence'])){
            if($postData['Tmptypabsence']['libelle']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Tmptypabsence ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Tmptypabsence->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Tmptypabsences', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Tmptypabsences', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Tmptypabsence->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Tmptypabsences', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION TYPE ABSENCE':'MODIFICATION TYPE ABSENCE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE TYPE ABSENCE'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Tmptypabsence->find('all', array('conditions'=>array(array($this->Tmptypabsence->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Tmptypabsences';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Tmptypabsence']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Tmptypabsence->delete($ids);
