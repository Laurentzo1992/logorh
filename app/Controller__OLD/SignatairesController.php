...
class SignatairesController extends AppController{	
	var $paginate = array(
		'Signataire'=>array(
			'model'=>'Signataire','sort'=>'matricule', 'signataire'=>'ASC',
	...
	var $uses = array('User','Agdossier','Paramnatcontrat','Paramfonction','Paramstatactivation');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Signataires'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SIGNATAIRES <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('signataires', $this->paginate('Signataire'));
		...
		$this->set('paramfonctions', $this->Paramfonction->find('list', array('list'=>array('id', 'nom_fonction'), 'conditions'=>array("Paramfonction.id in (28,18)"),'order'=>'nom_fonction ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Signataire']['submit']) && isset($postData['Signataire'])){
            if($postData['Signataire']['agdossier_id']<>'' &&
			   $postData['Signataire']['signature']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Signataire ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){
					/*-----------------------------------*/
                    $tmp = $this->Agdossier->find('all', array('conditions'=>array('Agdossier.id='.$postData['Signataire']['agdossier_id']), 'recursive'=>0));
		            $matricule = $tmp[0]['Agdossier']['ag_matricule'];
                    /*-----------------------------------*/	
                   // print_r($matricule);
                    $postData['Signataire']['matricule']  = $matricule;			
				    $saveId = $this->Signataire->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Signataires', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Signataires', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Signataire->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Signataires', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION SIGNATAIRE':'MODIFICATION SIGNATAIRE'));
	...
	public function search7() {
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Signataires', 'view'=>'index7', 'params'=>array()),
				'options'=>array('class'=>'toolbarItem back')
			);
		}else{
			$toolbar['Retour'] = array(
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Signataire->find('all', array('conditions'=>array(array($this->Signataire->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Signataires';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Signataire']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Signataire->delete($ids);
