...
class CarsouscriteresController extends AppController{	
	var $paginate = array(
		'Carsouscritere'=>array(
			'model'=>'Carsouscritere','sort'=>'id', 'carsouscritere'=>'ASC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramsociopro','Paramtypefonction','Paramtypevaluation','Carcritere');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Carsouscriteres'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'SOUS-CRITERES D\'APPRECIATION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('carsouscriteres', $this->paginate('Carsouscritere'));
		$this->set('paramsociopros', $this->Paramsociopro->find('list', array('list'=>array('id','libelle'), 'order'=>'libelle ASC')));
        ...
        $this->set('typevals', $this->Carcritere->find('list', array('list'=>array('id','paramtypevaluation_id'), 'order'=>'id ASC')));

        $this->set('paramtypevaluations', $this->Paramtypevaluation->find('list', array('list'=>array('id','libelle'), 'order'=>'id ASC')));

	    /* $this->set('responsables',$this->Responsable->find('list',array('list'=>array('id','VirtualFields.name')
		, 'fields'=>'id, CONCAT(nom_resp," ",prenom_resp) as name')));*/
	}
	...
	public function edit() {
		...
		if(isset($postData['Carsouscritere']['submit']) && isset($postData['Carsouscritere'])){
            if($postData['Carsouscritere']['libelle']<>'' &&
               $postData['Carsouscritere']['carcritere_id']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Carsouscritere ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Carsouscritere->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Carsouscriteres', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Carsouscriteres', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Carsouscritere->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Carsouscriteres', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION CRITERE D\'APPRECIATION':'MODIFICATION CRITERE D\'APPRECIATION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE CRITERE D\'APPRECIATION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Carsouscritere->find('all', array('conditions'=>array(array($this->Carsouscritere->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Carsouscriteres';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Carsouscritere']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Carsouscritere->delete($ids);
