...
class EltsaltypefctsController extends AppController{	
	var $paginate = array(
		'Eltsaltypefct'=>array(
			'model'=>'Eltsaltypefct','sort'=>'paramtypefonction_id', 'eltsaltypefct'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	var $uses = array('User','Paramindemnite','Paramtypefonction');
	
	
	
	public function index() {
		$this->requestAction('Users' ,'loggedIn');				
		$accessLevel = $this->requestAction('Users' ,'access', array('Eltsaltypefcts'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'ELEMENT SALAIRE PAR TYPE DE FONCTION <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('eltsaltypefcts', $this->paginate('Eltsaltypefct'));
		//$this->set('codes', $this->Paramindemnite->find('list', array('list'=>array('code','code'), 'order'=>'code ASC')));
        
		$this->set('codes',$this->Paramindemnite->find('list',array('list'=>array('id','VirtualFields.name'), 'fields'=>'id, CONCAT(code," ",libelle) as name')));
		
		
		$this->set('typefonctions', $this->Paramtypefonction->find('list', array('list'=>array('id','libelle'), 'order'=>'libelle ASC')));
	...
	public function edit() {
		...
		if(isset($postData['Eltsaltypefct']['submit']) && isset($postData['Eltsaltypefct'])){
            if($postData['Eltsaltypefct']['code']<>'' &&
			   $postData['Eltsaltypefct']['paramtypefonction_id']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Eltsaltypefct ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Eltsaltypefct->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Eltsaltypefcts', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Eltsaltypefcts', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Eltsaltypefct->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Eltsaltypefcts', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION ELEMENT SALAIRE PAR TYPE DE FONCTION':'MODIFICATION ELEMENT SALAIRE PAR TYPE DE FONCTION'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE ELEMENT SALAIRE PAR TYPE DE FONCTION'));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Eltsaltypefct->find('all', array('conditions'=>array(array($this->Eltsaltypefct->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Eltsaltypefcts';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Eltsaltypefct']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Eltsaltypefct->delete($ids);
	...
	public function etat() {
		...
		$this->set('pageTitle', ('LISTE DES EltsaltypefctS'));
		//$this->set('toolbar', $toolbar);
		$this->set('Eltsaltypefcts', $this->paginate('Eltsaltypefct'));
