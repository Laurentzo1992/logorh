...
class ParampharmaciesController extends AppController{	
	var $paginate = array(
		'Parampharmacie'=>array(
			'model'=>'Parampharmacie','sort'=>'id', 'Parampharmacie'=>'DESC',
			'page'=>1, 'recursive'=>0
		),
	...
	public function index() {
		...
		$accessLevel = $this->requestAction('Users' ,'access', array('Parampharmacies'));
		if($accessLevel['view']){
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . 'PHARMACIE <span class="pageTitle">'.$name . SEP . $username.'</span>');
		$this->set('parampharmacies', $this->paginate('Parampharmacie'));
	...
	public function edit() {
		...
		if(isset($postData['Parampharmacie']['submit']) && isset($postData['Parampharmacie'])){
            if($postData['Parampharmacie']['nom_pharma']<>'')
            {
					/*********************************************************************/
					$log = ($this->getGetParam('id')?'Modification':'Creation') . ' Parampharmacie ' . 'id: ';
					if($accessLevel['view'] && $accessLevel['edit']){				
						$saveId = $this->Parampharmacie->save($postData);
						...
						if($saveId){
							...
							if($this->Session->check('return')){
								$this->redirect(array('controller'=>'Parampharmacies', 'view'=>'index'));
							}else{
								$this->redirect(array('controller'=>'Parampharmacies', 'view'=>'index'));
							}
						}else {
		...
		if($this->getGetParam('id')){
			if(!empty($postData)){
				...
				$this->data = $this->Parampharmacie->read($id);
			}
		}
		...
		if($this->Session->check('return')){
			$toolbar['Retour'] = array(
				'url'=>array('controller'=>'Parampharmacies', 'view'=>'index', 'params'=>array()),
		...
		$this->set('pageTitle', (!$this->getGetParam('id')? APP_DEFAULT_NAME . SEP . 'CR&Eacute;ATION PHARMACIE ':'MODIFICATION PHARMACIE'));
	...
	public function search() {
		...
		$this->set('pageTitle', APP_DEFAULT_NAME . SEP . ('RECHERCHE PHARMACIE '));
		$this->set('toolbar', $toolbar);
	}
	...
	public function del (){
		...
		if($accessLevel['view'] && $accessLevel['del'] && $this->getGetParam('id')){
			$data = $this->Parampharmacie->find('all', array('conditions'=>array(array($this->Parampharmacie->primaryKey=>$ids)), 'recursive'=>-1));
			$log = 'Suppression Parampharmacies';
			$dataList = array();
			
			foreach ($data as $d){
				$dataList[] = 'id:' . $d['Parampharmacie']['id'];
				
			}
			$log .= implode(', ', $dataList);		
			$this->requestAction('Logs' ,'record', $log);
			
			$this->Parampharmacie->delete($ids);
